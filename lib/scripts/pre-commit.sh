...
typescripts=$(echo "$files" | grep -e ".tsx\?$")
stylesheets=$(echo "$files" | grep -e ".styl$")

WARN=false
FAIL=false

if [ "$images" != "" -o "$jsons" != "" -o "$javascripts" != "" -o "$stylesheets" != "" ]; then
  echo ""
...
if [ "$typescripts" != "" ]; then
  for typescript in ${typescripts}; do
    result=$(./node_modules/.bin/eslint --parser typescript-eslint-parser ${typescript} -f compact | sed -e "s/^.*: //g")
    if [ "$(echo $result | grep Error)" = "" ]; then
      if [ "$(echo $result | grep Warning)" = "" -o "$(echo $result | grep 'File ignored because of a matching ignore pattern')" != "" ]; then
        echo "\t\033[32m✓ eslint:\t${typescript}\033[0m"
      else
        WARN=true
        echo "\t\033[33m⚠︎︎ eslint:\t${typescript}\033[0m"
        echo "$result"
      fi
    else
      FAIL=true
      echo "\t\033[31m✗ eslint:\t${typescript}\033[0m"
      echo "$result"
    fi
  done
fi
